...
class And(BooleanOperator):
    ...
    def __init__(self, left, right):
        super(And, self).__init__(next(counter))
        self.left = left
        self.right = right
        if DEBUG:
...
class Or(BooleanOperator):
    ...
    def __init__(self, left, right):
        super(Or, self).__init__(next(counter))
        self.left = left
        self.right = right
        if DEBUG:
...
class Not(BooleanOperator):
    ...
    def __init__(self, param):
        if type(param) == Atomic:
            super(Not, self).__init__(-param.name)
        else:
            super(Not, self).__init__(next(counter))
        self.param = param
        if DEBUG:
...
class Imp(BooleanOperator):
    ...
    def __init__(self, left, right):
        super(Imp, self).__init__(next(counter))
        self.left = left
        self.right = right
        if DEBUG:
...
class Equiv(BooleanOperator):
    ...
    def __init__(self, left, right):
        super(Equiv, self).__init__(next(counter))
        self.left = left
        self.right = right
        if DEBUG:
            print(self.name , " : " , self.left.name , " <=> " , self.right.name)
...
class Atomic(BooleanOperator):
    ...
    def __init__(self, name):
        if name not in names.keys():
            ...
            super(Atomic, self).__init__(names[name])
        else:
            super(Atomic, self).__init__(names[name])
        self.val = name
        if DEBUG:
